...
//      1) create a loop that will only break when a valid move is made
...
int getUserMove(int stones) {
    int userMove;
    while (true) {
        std::cout << "\nHow many stones would you like to take (1-3)? ";
        std::cin >> userMove;
...
int main() {
    ...
    //      pile
    int stones = 16;
    std::cout << "\nThere are " << stones << " stones in the pile." << std::endl;
    ...
    while (true) {
        ...
        int cpuMove = computerMove(stones);
        stones -= cpuMove;
		std::cout << "The computer takes " << cpuMove << " stones." << std::endl;
